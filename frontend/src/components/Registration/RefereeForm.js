...
const RefereeForm = ({ user, registrationData, onCompleteRegistration }) => {
  ...
  useEffect(() => {
    ...
    if (storedEditData) {
      try {
        ...
        if (editData.isEditMode && editData.userType === 'referee') {
          setIsEditMode(true);
  ...
  /// In RefereeForm.js - Update the districts fetch useEffect
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (orderId && userType === 'referee' && !isEditMode) {
  ...
  const nextStep = () => {
    ...
    if (currentStep === 2) {
      ...
      if (!formData.level) {
        setMessage('Please select your referee level');
        return;
      }
    }
  ...
  const updateRefereeApplication = async () => {
    try {
      ...
      console.log('Updating referee application with ID:', editApplicationId, updateData);

      const response = await axios.put(
        `${API_BASE_URL}/api/admin/update-application/referee/${editApplicationId}`,
        updateData,
        {
          headers: {
      ...
      console.error('Error updating referee application:', error);
    ...
    try {
      ...
      if (paymentSession.success) {
        ...
        const checkoutOptions = {
          ...
          returnUrl: `${window.location.origin}/payment-success?order_id=${paymentSession.data.order_id}&user_type=referee`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        await updateRefereeApplication();
      } else {
  ...
  return (
    <div className="referee-form-container">
      <div className="form-header">
        <h1>
          {isEditMode ? '✏️ Edit Referee Application' : 'Referee Registration'}
        ...
        <p>
          {isEditMode
            ...
            : 'Complete your profile to join Delhi Fencing Association as a Referee'}
        </p>
        {isEditMode && (
              ...
              <div className="form-row">
                <div className="form-group full-width">
                  <div className="info-box">
                    ...
                    <ul>
                      <li>You can update your personal and referee information</li>
